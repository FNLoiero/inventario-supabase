...
export const categories: Category[] = [
  ...
  { id: 'cat-tech', name: 'Tecnología', color: '#59d4a9' },
...
export const products: Product[] = [
  {
    ...
    minStock: 6,
    status: 'low_stock',
    description: 'Lámpara de mesa con luz cálida y base de madera.',
  ...
  },
  {
    ...
    minStock: 5,
    status: 'active',
    description: 'Mouse inalámbrico ergonómico con clic silencioso.',
...
export function getInventoryStats(currentProducts: Product[]) {
  ...
  const lowStock = currentProducts.filter((product) => product.status === 'low_stock').length;
  const outOfStock = currentProducts.filter((product) => product.status === 'out_of_stock').length;
  const totalUnits = currentProducts.reduce((sum, product) => sum + product.stock, 0);
